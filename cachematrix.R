...
makeCacheMatrix <- function(x = matrix()) {
    ...
    ## When getinverse is NULL, it's a sign that the inverse hasn't been calculated.
    cache <- NULL
    ...
    ## 4. This function returns the inverse currently set 
...
cacheSolve <- function(x, ...) {
    ...
    ## cacheSolve takes advantage of the 4 functions of 'x' to logically determine whether to run the Solve() function
    
    ## Run the getinverse function of the special matrix
    result <- x$getinverse()
    ...
    ## Run only if result is NULL, using the functions of the special matrix
